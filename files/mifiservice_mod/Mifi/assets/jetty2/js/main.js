...
function init() {
  ...
  request(param, function (data) {
    ...
    if (flag == "1") {
      ...
      if (result.netstatus) {
        ...
        else $("#activate").html(mifi_translate("disconnected"));
      }
    } else {
      //����
  ...
  if (getCookie("language") == "Chinese") {
    ...
    var helpEn = document.getElementById("helpEn");
    var helpZh = document.getElementById("HelpZh");
    if (helpEn) helpEn.style.display = "none";
    if (helpZh) helpZh.style.display = "inline";
    ...
    if (body6) body6.style.display = "inline";
    if (body7) body7.style.display = "none";
  } else {
    $("#zh").html("Chinese");
    $("#en").html("English");
    $("#ru").html("Russion");
